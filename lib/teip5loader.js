...
async function parseVolume(volumeNode,jing_id){
    var volume_id=select("@xml:id",volumeNode,true).nodeValue;
    var volume_title=select("@n",volumeNode,true).nodeValue;
...
function getVolume(xmldoc){
    return select('/tei:TEI/tei:text/tei:body/tei:div[@type="volume"]',xmldoc)
}
